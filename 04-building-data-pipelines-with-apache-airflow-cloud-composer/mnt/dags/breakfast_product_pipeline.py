...
# not recommand เพราะ มัน exec (เวลา airflow เรียกรันtaskนี้ มันจะ hit database) ทำให้ performance แย่ เพราะต้องมาเรียก ฟังชันทุกครั้ง มันควรเป็น เมื่อ tricker ถึงจะเรียกใช้
# DAGS_FOLDER = Variable.get("dags_folder") => ใช้ kwargs กำหนดที่ PythonOperator โดยกำหนด para เพิ่ม
...
def _load_products_to_gcs(dags_folder=""):
    ...
    file_path = f"{dags_folder}/datasets/breakfast_products.csv"
...
def _load_products_from_gcs_to_bigquery(dags_folder=""):
    DATA_FOLDER = "data"
    BUSINESS_DOMAIN = "breakfast"
    location = "asia-southeast1"
...
with DAG(
    ...
    load_products_from_gcs_to_bigquery = PythonOperator(
        ...
        python_callable=_load_products_from_gcs_to_bigquery,
        op_kwargs={
            "dags_folder": "{{var.value.dags_folder}}"
        },
    )
