...
def _extract_from_mysql():
    hook = MySqlHook(mysql_conn_id="pim_netflix_mysql_conn") # บอกด้วยว่าต่อไปที่ใคร  #สร้างที่ airflow conn
    ...
    df = pd.read_sql("select * from Netflix_Userbase_utf_already_preprocessed_dates", con=conn)
    # print(df.head())
    local_file_name = "Netflix_Userbase_utf_already_preprocessed_dates.csv" #
...
def _load_to_gcs():
    ...
    keyfile_gcs = f"/opt/airflow/dags/pim-netflix-load-to-gcs-410506-a515b5ff892c.json" # ใช้ path ของ container ก็คือ /opt/airflow/ add both
...
def _load_from_gcs_to_bigquery():
    ...
    keyfile_bigquery = "/opt/airflow/dags/pim-netflix-load-from-gcs-to-bigquery-410506-3ae98eea6a59.json"
...
with DAG(
    "netflix_from_mysql_to_bigquery_pipeline",
    default_args=default_args,
    schedule=None,
    tags=["netflix", "mysql", "bigquery"],
):
